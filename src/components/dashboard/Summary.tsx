...
import { Target, Flame, Edit, Check } from 'lucide-react';
...
export default function Summary({ totalCalories, calorieTarget, onTargetChange }: SummaryProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [newTarget, setNewTarget] = useState(calorieTarget);

  const remainingCalories = calorieTarget - totalCalories;
  const progress = calorieTarget > 0 ? (totalCalories / calorieTarget) * 100 : 0;
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-center">
            ...
            <div className="p-4 bg-accent/10 rounded-lg">
              ...
              <p className={cn("text-2xl font-bold", remainingCalories < 0 && "text-destructive")}>
